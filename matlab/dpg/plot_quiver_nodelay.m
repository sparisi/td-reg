...
colors = [1 0.2 0.2; 1 0.5 0.2; 0.2 0.5 1; 0.49400,0.18400,0.55600; 0.46600,0.67400,0.18800; 0.4 1 0; 0.96, 0.73, 1.0];
fontsize1 = 10;
...
for i = 1 : 1 : size(theta_history_tdreg,2)-1
    ...
    dp = p2-p1;
    quiver(p1(1),p1(2),dp(1),dp(2),0,'Color',colors(6,:),'LineWidth',2,'MarkerSize',10,'MaxHeadSize',1)

    p1 = [theta_history_td3reg(1,i) theta_history_td3reg(4,i)];
    p2 = [theta_history_td3reg(1,i+1) theta_history_td3reg(4,i+1)];
    dp = p2-p1;
    quiver(p1(1),p1(2),dp(1),dp(2),0,'Color',colors(7,:),'LineWidth',2,'MarkerSize',10,'MaxHeadSize',1)
end
...
name = 'td3';
load([folder(3:end) name '_' num2str(idx_trial) '.mat'])
hl(2) = plot(theta_history(1,1),theta_history(4,1),'linewidth', 3, 'color', colors(4,:));
hold on;

name = 'td3reg';
load([folder(3:end) name '_' num2str(idx_trial) '.mat'])
hl(3) = plot(theta_history(1,1),theta_history(4,1),'linewidth',3, 'color', colors(5,:));
hold on;

name = 'td3_nodelay';
load([folder(3:end) name '_' num2str(idx_trial) '.mat'])
hl(4) = plot(theta_history(1,1),theta_history(4,1),'linewidth',3, 'color', colors(6,:));
hold on;

name = 'td3reg_nodelay';
load([folder(3:end) name '_' num2str(idx_trial) '.mat'])
hl(5) = plot(theta_history(1,1),theta_history(4,1),'linewidth',3, 'color', colors(7,:));
hold on;
...
hl_dpg1 = legend(hl, 'DPG TD-REG', 'TD3', 'TD3 TD-REG', 'TD3 NO-DELAY', 'TD3 TD-REG NO-DELAY', 'location', 'northwest');
