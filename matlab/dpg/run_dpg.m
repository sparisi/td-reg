function run_dpg(trial, folder_save, varargin)
% Simple DPG with policy target and Q target.
...
while totsteps < minsteps + stepslearn
    ...
    while ( (step < maxsteps) && ~done )
        ...
        if totsteps > minsteps
            ...
            a_pi = theta * bfs_s;
            an_pi = theta_t * bfs_sn; % Use target policy for next action
            bfs_s_a = data.bfs_s_a(:,mb);
            bfs_sn_anpi = basis_q(sn, an_pi);
            bfs_d_s_api = basis_q_da(s,a_pi);
            
            q = omega' * bfs_s_a;
            q_t = omega_t' * bfs_sn_anpi; % Use target Q-function
...
save([folder_save 'dpg_' num2str(trial)], 'l2_diff_history', 'J_history', 'td_history', 'td_true_history', 'theta_history', 'omega_history', 'df_theta_history', 'dg_omega_history')
